...
/// An [`$exitcode`] is "generated by a process when exiting."
///
/// It is used as the parameter to the [`proc_exit`](crate::api::Api::proc_exit) function.
///
/// [`$exitcode`]: https://github.com/WebAssembly/WASI/blob/snapshot-01/phases/snapshot/witx/typenames.witx#L592
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq, PartialOrd, Ord)]
...
impl ExitCode {
    ...
    /// Returns `true` if the [`ExitCode`] is `0` indicating a [`SUCCESS`]. "The meanings of other
    /// (non-zero) values is dependent on the environment."
    ///
    ...
    /// Converts this [`ExitCode`] into one that can be passed to the [Rust standard library] to
    /// terminate the process.
    ///
    /// On non-`windows` platforms, any [`ExitCode`] values greater than [`u8::MAX`] are simply
    /// replaced with [`u8::MAX`].
    ///
    /// [Rust standard library]: std
    #[cfg(feature = "std")]
    pub fn to_exit_code_lossy(self) -> std::process::ExitCode {
        #[cfg(target_family = "windows")]
        return std::os::windows::process::ExitCode::Ext::from_raw(self.0);

        #[cfg(not(target_family = "windows"))]
        return std::process::ExitCode::from(u8::try_from(self.0).unwrap_or(u8::MAX));
    }
}
...
/// A [`$signal`] condition.
///
/// [`$signal`]: https://github.com/WebAssembly/WASI/blob/snapshot-01/phases/snapshot/witx/typenames.witx#L595C1-L692C1
enum Signal(u8) = {
    /// "No signal."
    None = 0,
    /// "Hangup."
    ...
    /// # Action
    ///
    /// "Continues executing, if stopped."
    Cont = 17,
    /// "Stop executing."
    ///
    /// # Action
